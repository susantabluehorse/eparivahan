        ...
        if(accessToken.length > 0){
            ...
            if(mobileVali.passes()===true){
                ...
                if(TrackingDetails.length > 0){ //query result length check
                    TrackingDetails.forEach(function(k,p){
                        ...
                        TrackingDetails[p]['trackingDetails']['latitude'] = k.latitude;
                        TrackingDetails[p]['trackingDetails']['longitude'] = k.longitude;
                        TrackingDetails[p]['trackingDetails']['timeStamp'] = k.time_stamp;
            ...
            if(mobileVali.passes()===true){
                ...
                if(TrackingList.length > 0) { //query result length check
                    TrackingList.forEach(function(k,p){
                        ...
                        delete TrackingList[p].other_mobile_number;
                    });
            ...
            if(mobileVali.passes()===true){
                ...
                if(SearchOrganization.length > 0) { //query result length check
                    SearchOrganization.forEach(function(k,p){
                        ...
                        delete SearchOrganization[p].other_mobile_number;
                    });            
                    res.status(200).json({success: true,data:SearchOrganization}); //Return json with data or empty
                } else {
                    res.status(200).json({ success: false,data: "No data found!"}); // Return json with error massage
...
exports.updateTrackingDetails = async function(req, res, next) {
    ...
    if(header.passes()===true){
        ...
        if(accessToken.length > 0){
            ...
            if(mobileVali.passes()===true){
                var updateTrackingDetails =await sequelize.query("SELECT `t`.`id` AS Id FROM `tracking_details` AS t LEFT JOIN `tracking_mappers` AS tm ON `tm`.`tracking_id`=`t`.`tracking_id` LEFT JOIN `users` AS u ON `u`.`id`=`tm`.`user_id` LEFT JOIN `branchs` AS b ON `b`.`id`=`tm`.`branch_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`b`.`enterprise_id` LEFT JOIN `vehicle_details` AS v ON `v`.`enterprise_id`=`e`.`id` WHERE `t`.`tracking_id`="+trackingId+" AND `e`.`id`="+organizationId+" AND `u`.`id`="+userId+"",{ type: Sequelize.QueryTypes.SELECT });
                if(updateTrackingDetails.length > 0){
                    var timeStamp = trackingDetails.timeStamp;
                    var latitude = trackingDetails.latitude;
                    var longitude = trackingDetails.longitude;
