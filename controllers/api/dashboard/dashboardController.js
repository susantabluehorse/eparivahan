...
exports.getTrackingCount = async function(req, res, next) {
    ...
    if(mobileVali.passes()===true){
        if(role=='admin'){ // Admin get all data
            var TrackingCount =await sequelize.query("SELECT COALESCE(SUM(IF(`t`.`active_status` = 'active', 1, 0)),0) AS active, COALESCE(SUM(IF(`t`.`status` = 'tracked', 1, 0)),0) AS tracked, COALESCE(SUM(IF(`t`.`status` = 'not-tracked', 1, 0)),0) AS 'not-tracked', COALESCE(SUM(IF(`t`.`status` = 'completed', 1, 0)),0) AS completed FROM `tracking_details` AS t LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` WHERE DATE(`t`.`start_date`)>='"+fromDate+"' AND DATE(`t`.`start_date`) <='"+toDate+"'",{ type: Sequelize.QueryTypes.SELECT });
    ...
    if(mobileVali.passes()===true){
        if(role=='admin'){ // Admin get all data
            if(type=='month'){ // Fetch Data Month Wise
                var Failed =await sequelize.query("SELECT COUNT(`t`.`id`) AS totalFailed, {fn MONTHNAME(`t`.`start_date`)} AS month, COUNT(`t`.`start_date`) AS count FROM `tracking_details` AS t LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`status`='failed'  AND DATE(`t`.`start_date`)>='"+fromDate+"' AND DATE(`t`.`start_date`) <='"+toDate+"'"+shipperIdQ+" GROUP BY MONTH(`start_date`)",{ type: Sequelize.QueryTypes.SELECT });
            ...
            if(type=='month'){ // Fetch Data Month Wise
                var Failed =await sequelize.query("SELECT COUNT(`t`.`id`) AS totalFailed, {fn MONTHNAME(`t`.`start_date`)} AS month, COUNT(`t`.`start_date`) AS count FROM `tracking_details` AS t LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`status`='failed' AND `t`.`tracked_by_user_id`="+userId+" AND DATE(`t`.`start_date`)>='"+fromDate+"' AND DATE(`t`.`start_date`) <='"+toDate+"'"+shipperIdQ+" GROUP BY MONTH(`start_date`)",{ type: Sequelize.QueryTypes.SELECT });
            } else { // Fetch Data Date Wise
                var Failed =await sequelize.query("SELECT COUNT(`t`.`id`) AS totalFailed, DATE_FORMAT(`t`.`start_date`,'%d-%m-%Y') AS month, COUNT(`t`.`start_date`) AS count FROM `tracking_details` AS t LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`status`='failed' AND `t`.`tracked_by_user_id`="+userId+" AND DATE(`t`.`start_date`)>='"+fromDate+"' AND DATE(`t`.`start_date`) <='"+toDate+"'"+shipperIdQ+" GROUP BY DATE(`start_date`)",{ type: Sequelize.QueryTypes.SELECT });              
            }
        }
...
exports.getTrackingSearch = async function(req, res, next) {
    ...
    if(mobileVali.passes()===true){
        if(trackingId!='' && trackingId!=undefined){
            ...
            Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber, `b`.`id` AS branchId, `b`.`address` AS branchAddress,`b`.`contact_number` AS branchContact, `l`.`pickup_location_latitude`, `l`.`pickup_location_longitude`, `l`.`drop_location_latitude`, `l`.`drop_location_longitude` FROM `tracking_details` AS t LEFT JOIN `tracking_mappers` AS tm ON `tm`.`tracking_id`=`t`.`tracking_id` LEFT JOIN `loads` AS l ON `l`.`id`=`tm`.`load_id`  LEFT JOIN `branchs` AS b ON `b`.`id`=`tm`.`branch_id` LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`vehicle_number`='"+vehicleNnumber+"'",{ type: Sequelize.QueryTypes.SELECT });
        } else if(organizationId!='' && organizationId!=undefined){
            Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber, `b`.`id` AS branchId, `b`.`address` AS branchAddress,`b`.`contact_number` AS branchContact, `l`.`pickup_location_latitude`, `l`.`pickup_location_longitude`, `l`.`drop_location_latitude`, `l`.`drop_location_longitude` FROM `tracking_details` AS t LEFT JOIN `tracking_mappers` AS tm ON `tm`.`tracking_id`=`t`.`tracking_id` LEFT JOIN `loads` AS l ON `l`.`id`=`tm`.`load_id`  LEFT JOIN `branchs` AS b ON `b`.`id`=`tm`.`branch_id` LEFT JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` LEFT JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `e`.`id`='"+organizationId+"'",{ type: Sequelize.QueryTypes.SELECT });
