...
exports.getTrackingSearch = async function(req, res, next) {
    ...
    var Tracking_Search='';
    if(trackingId!=''){
        Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `t`.`longitude`, `t`.`latitude`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber FROM `tracking_details` AS t INNER JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` INNER JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`id`='"+trackingId+"'",{ type: Sequelize.QueryTypes.SELECT });
        if(Tracking_Search.length <= 0){
            Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `t`.`longitude`, `t`.`latitude`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber FROM `tracking_details` AS t INNER JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` INNER JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`vehicle_number`='"+trackingId+"'",{ type: Sequelize.QueryTypes.SELECT });
            if(Tracking_Search.length <= 0){
                Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `t`.`longitude`, `t`.`latitude`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber FROM `tracking_details` AS t INNER JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` INNER JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `e`.`id`='"+trackingId+"'",{ type: Sequelize.QueryTypes.SELECT });
                if(Tracking_Search.length <= 0){
                    Tracking_Search = await sequelize.query("SELECT `t`.`tracking_id`, `t`.`longitude`, `t`.`latitude`, `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber FROM `tracking_details` AS t INNER JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` INNER JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `t`.`driver_mobile_number`='"+trackingId+"'",{ type: Sequelize.QueryTypes.SELECT });
                }
            }
        }
        if(Tracking_Search.length > 0){
...
exports.getActiveInactiveShipper = async function(req, res, next) {
    ...
    if(type !='' && count !='' && fromDate !='' && toDate !=''){
        var ActiveInactiveShipper = await sequelize.query("SELECT `e`.`id` AS OrganizationId, `e`.`primary_contact_no` AS OrganizationNumber, COUNT(`t`.`id`) AS totalTracking FROM `tracking_details` AS t INNER JOIN `users` AS u ON `u`.`id`=`t`.`tracked_by_user_id` INNER JOIN `enterprises` AS e ON `e`.`id`=`u`.`enterprise_id` WHERE `e`.`status`='"+type+"' AND DATE(`t`.`start_date`)>='"+fromDate+"' AND DATE(`t`.`start_date`) <='"+toDate+"' LIMIT "+count+"",{ type: Sequelize.QueryTypes.SELECT });
