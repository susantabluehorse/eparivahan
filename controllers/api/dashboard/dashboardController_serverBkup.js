...
exports.getTrackingAdd = async function(req, res, next) {
    ...
    if(header.passes()===true){
        var accessToken =await sequelize.query("SELECT `id`,`remember_token` FROM `users` WHERE `remember_token`='"+access_token+"'",{ type: Sequelize.QueryTypes.SELECT });
        if(accessToken.length > 0){
            const { userId, organizationId, branchId, loadId, startDate, vehicleNumber, driverNumber, driverName, otherMobileNumber, driverMobileOnTrack, from, to, totalTrackCount, comment } = req.body;
            ...
            if(mobileVali.passes()===true){
                ...
                var createTracking = await sequelize.query("INSERT INTO `tracking_details`(`tracking_count`, `driver_mobile_number`, `comment`, `tracked_by_user_id`, `driver_name`, `vehicle_number`,`other_mobile_number`, `driver_mobile_on_track`, `from_location`, `to_location`, `start_date`, `created_by`) VALUES ("+totalTrackCount+", '"+driverNumber+"', '"+comment+"', "+userId+", '"+driverName+"', '"+vehicleNumber+"', '"+otherMobileNumbers+"', "+driverMobileOnTrack+", '"+froms+"', '"+tos+"', '"+startDate+"', "+userId+")",{ type: Sequelize.QueryTypes.INSERT });
